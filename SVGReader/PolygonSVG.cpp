...
static vector<PointF> parsePointsString(string pointsStr) {
    ...
    for (size_t i = 0; i < pointsStr.length(); ++i) {
        if (pointsStr[i] == ',' || pointsStr[i] == '\r' || pointsStr[i] == '\n') {
            pointsStr[i] = ' ';
        }
    }
...
void PolygonSVG::draw(Graphics& graphics, const DefinitionsSVG& defs) {
    ...
    vector<PointF> polyPoints = parsePointsString(points);
    ...
    GraphicsPath path;
    path.AddPolygon(polyPoints.data(), (int)polyPoints.size());
