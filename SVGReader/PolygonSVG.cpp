...
void PolygonSVG::read(xml_node<>* node) {
    ...
    for (xml_attribute<>* attr = node->first_attribute(); attr; attr = attr->next_attribute()) {
        ...
        else if (type == "opacity") { opacity.setOpacity(stof(value)); hasOpacity = true; }
        else if (type == "fill") {
        ...
        else if (type == "fill-opacity") { fill.setFillOpacity(stof(value)); hasFillOpacity = true; }
        else if (type == "stroke") { stroke.setStrokeColor(getRGB(value)); hasStrokeColor = true; }
        else if (type == "stroke-width") { stroke.setStrokeWidth(stof(value)); hasStrokeWidth = true; }
        else if (type == "stroke-opacity") { stroke.setStrokeOpacity(stof(value)); hasStrokeOpacity = true; }
        else if (type == "transform") parseTransform(value);
    }
}
...
void PolygonSVG::draw(Graphics& graphics, const DefinitionsSVG& defs) {
    ...
    if (fillBrush) graphics.FillPolygon(fillBrush, polyPoints.data(), (int)polyPoints.size(), FillModeWinding);
    if (strokePen) graphics.DrawPolygon(strokePen, polyPoints.data(), (int)polyPoints.size());
